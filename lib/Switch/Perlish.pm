...
require Exporter;
@ISA     = 'Exporter';
@EXPORT  = qw/ switch case default fallthrough /;
$VERSION = '1.0.0';
...
use vars qw/ $MATCH $TOPIC $SWITCH $CASE /;
...
sub left_ok { ref($_[0]) and UNIVERSAL::isa($_[0], SUCCESS) }
...
sub _exec_block {
  ...
  return( defined wantarray ? wantarray ? @ret : $ret[-1] : () )
...
sub case {
  ...
  return
    ## store the match() result somewhere?
    unless Switch::Perlish::Smatch->match($TOPIC, $MATCH);
...
If you're unfamiliar with switch then this is the best place to start. A switch
statement is essentially syntactic sugar for an if/elsif/else chain where the
same C<$variable> is tested in every conditional e.g:
...
last C<else> block if nothing matched. An equivalent switch statement (using
...
call the C<fallthrough> subroutine explicitly.
...
L<Switch::Perlish::Smatch::Comparators>, which lists all the default
comparators.
...
C<$block> and exit from C<switch>. I<NB>: this cannot be called outside of
C<switch>, if you want to use I<smart matching> functionality, see.
